...
class BallFinder:
    ...
    def callback_rgb(self,data):
        ...
        # Perform color segmentation to find the red object
        red_bbox = self.get_bounding_box(cv_image)
        if not red_bbox:
    ...
    def post_process(self):
        ...
        try: 
            trans, rot = self.tfsub.waitForTransform("/camera_rgb_optical_frame", "/world",  rospy.Time(0), rospy.Duration(3.0))
            T_cam_world = tf.TransformerROS.fromTranslationRotation(trans,rot ) 
            print("T cam world {}".format(T_ball_world))
        except: pass 
# 
        ...
        if ball_distance_from_base <= self.safe_dis_from_base: 
            self.publish_odometry("RED", x, y, z, self.velocity, self.current_time)
        else: 
            pass
   ...
   # Publish point
    def publish_odometry(self, name, x, y , z, velocity, time):  
        ...
        # trackBall.header.stamp.secs = double(time)
